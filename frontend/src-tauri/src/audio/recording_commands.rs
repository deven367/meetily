...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    // Load recording preferences to get auto_save, device preferences, and save folder
    let (auto_save, preferred_mic_name, preferred_system_name, save_folder) =
        match super::recording_preferences::load_recording_preferences(&app).await {
            Ok(prefs) => {
                ...
                (prefs.auto_save, prefs.preferred_mic_device, prefs.preferred_system_device, prefs.save_folder.clone())
            }
            Err(e) => {
...
pub async fn start_recording_with_devices_and_meeting<R: Runtime>(
    ...
    let (auto_save, save_folder) = match super::recording_preferences::load_recording_preferences(&app).await {
        Ok(prefs) => {
            ...
            (prefs.auto_save, prefs.save_folder.clone())
        }
        Err(e) => {
            warn!("Failed to load recording preferences, defaulting to auto_save=true: {}", e);
            (true, super::recording_preferences::get_default_recordings_folder()) // Default to saving if preferences can't be loaded
        }
    };
